...
function Home() {
    ...
    const fetchUserDetails = () => {
      const userSession = JSON.parse(sessionStorage.getItem("userSession"));
      if(userSession) {
        axios.get("http://poker-service.azurewebsites.net/user-details", { params: { roomId: userSession.roomId } }).then((response) => {
    ...
    const castVote = (option) => {
      ...
      setVoteStatus(true);
      setVoteValue(option);
      axios.post("http://poker-service.azurewebsites.net/cast-vote", { ...request }).then((response) => {
        if(response && response.status === 201) {
    ...
    const handleRevealCard = () => {
      axios.get("http://poker-service.azurewebsites.net/reveal-card").then(res => {
        if(res.status === 200) {
    ...
    const handleLeaveRoom = () => {
      axios.get("http://poker-service.azurewebsites.net/leave-room").then(res => {
        if(res.status === 201) {
